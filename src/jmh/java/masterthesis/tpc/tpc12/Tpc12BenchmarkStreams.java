...
@State(Scope.Thread)
public class Tpc12BenchmarkStreams {
    ...
    public List<Tpc12ResultRow> parallelStreams() throws Exception {
        ...
        Map<String, List<IntSummaryStatistics>> collect = StreamUtils.innerJoin(store.getOrders(), store.getLineItems(), pair -> pair.left.equals(pair
                .right.order)).parallel()
                .filter(pair -> {
                            if (i.incrementAndGet() % 1000 == 0) {
                                System.out.println("Filter: " + i.incrementAndGet());
                            }
                            return (Objects.equals(pair.right.shipMode, "MAIL") || Objects.equals(pair.right.shipMode, "SHIP"))
                                    &&
